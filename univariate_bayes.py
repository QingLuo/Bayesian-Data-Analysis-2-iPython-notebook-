...
Revised Mar 12, 2015 by Qing Luo
"""
...
class UnivariateBayesianInference(object):
    ...
    #posterior mean
    def mean(self):
        return np.trapz(self.post_pdf*self.param_grid,dx=self.delta)  

    #posterior std
    def std(self):
        variance=np.trapz(((self.param_grid-self.mean())**2)*self.post_pdf,dx=self.delta)
